...
public class NextLong extends LatticeCall<JRand> {
	...
	public void build(Lattice<JRand> lattice) {
		...
		boolean minSignBit = (this.min & Mth.pow2(31)) != 0; //Would a long having value min run into a negative (int) cast
		boolean maxSignBit = (this.max & Mth.pow2(31)) != 0; //Would a long having value max run into a negative (int) cast
		...
		if(this.min >>> 32 == this.max >>> 32) { //Can we even talk about the second seed?
			lattice.processCall(Next.inBitsRange(32, this.min & Mth.mask(32), (this.max & Mth.mask(32)) + 1));
		} else {
